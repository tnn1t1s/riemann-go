...
The expired event's `ttl` is therefore the model default of 60 rather than the expiring entry's ttl. Nothing re-indexes it unless a rule routes it to an index leaf, in which case the insert deletes rather than stores, per the indexing section above.
