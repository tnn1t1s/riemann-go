...
2. **Resolved 2026-09-17: `throttle`'s window is per fork key, opened by the first event for that key.** `SPEC.md` property 9 now states it, replacing wording that read as a sliding window. Two identities under one `by` do have windows offset from each other, and a rule author reasoning about a fleet-wide bound gets `limit` per key rather than `limit` overall; that is the cost of an anchor a scenario can assert against. The rejected alternative, upstream's division of the wall clock into bins, makes the same stimulus produce different firings depending on where a run falls against the grid.

3. **Resolved 2026-09-17: `otherwise` is required.** A `splitp` without it is rejected at `PUT` with `400`, per `SPEC.md`'s combinator table. No event falls off the node silently, which is what `INVARIANTS.md` I5 requires, and the rule author states where a miss goes rather than inheriting one of upstream's two disagreeing answers.
...
8. **Resolved 2026-09-17: bounded.** `SCALE.md` names `stable.buffer_capacity`, 1024 events per fork key, policy drop-oldest, each eviction counted as `riemann.rule.discarded`. The capacity is provisional, since nothing has measured a real buffer depth.
